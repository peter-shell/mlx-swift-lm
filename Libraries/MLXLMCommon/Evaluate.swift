...
struct TokenRing {
    ...
    ///
    /// Flattens first so 2D VLM/multimodal prompts shaped `[1, N]` count as `N`
    /// tokens (not batch size 1). Measuring `dim(0)` before flatten malformed the
    /// ring and crashed on the next `append` with a broadcast shape error.
